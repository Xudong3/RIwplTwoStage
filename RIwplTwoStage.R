...
###uninformative
estimator_PL<-fit_PL(TwostageSRSWORSample$y, TwostageSRSWORSample$cluster, TwostageSRSWORSample$x, pars=c(0,0,1,1))
estimator_PL

###informative sampling
estimatoris_PL<- fit_PL(TwostageSRSWORSampleis$y, TwostageSRSWORSampleis$cluster, TwostageSRSWORSampleis$x, pars=c(0,0,1,1))
estimatoris_PL
   ...
   for (i in 1:(n-1)){
      ...
      for(j in js){
         ...
         incrementdaij=wdalpha(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),
                               tau2=exp(theta[4]), pos[i], pos[j],sc[i], sc[j], n1, N1, n2infor,N2)
         ...
         incrementdakl=wdalpha(y[k],y[l],g[k],g[l],x[k],x[l], alpha=theta[1],beta=theta[2],
                               sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[k], pos[l], sc[k], sc[l], n1, N1, n2infor,N2)
...
for(i in 1:LOTS){
   ...
   rb<-fit_PL(TwostageSRSWORSample$y, TwostageSRSWORSample$cluster, TwostageSRSWORSample$x, pars=c(0,0,1,1))
   ...
   rbis<-fit_PL(TwostageSRSWORSampleis$y, TwostageSRSWORSampleis$cluster, TwostageSRSWORSampleis$x, pars=c(0,0,1,1))
