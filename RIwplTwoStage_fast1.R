...
estimatoris_PL<- fit_PL(TwostageSRSWORSampleis$y, TwostageSRSWORSampleis$cluster, TwostageSRSWORSampleis$x, pars=truevalue)
estimatoris_PL
...
estimatoris_WPL=fit_WPL(TwostageSRSWORSampleis$y, TwostageSRSWORSampleis$cluster,TwostageSRSWORSampleis$x,
...
for(i in 1:LOTS){
   ...
   rbis<-fit_PL(TwostageSRSWORSampleis$y, TwostageSRSWORSampleis$cluster, TwostageSRSWORSampleis$x,  pars=truevalue) 
...
vardf<- matrix(c( apply(PS_PL, 2, median),apply(PS_PL, 2, mad),  apply(sqrt_diagJ_PL, 2, median),
                  apply(PS_WPL, 2, median),apply(PS_WPL, 2, mad),  apply(sqrt_diagJ_WPL, 2, function(x) median(x,na.rm=TRUE))),ncol=6)
vardf<-round(vardf,2)
...
dfis<- matrix(c(apply(Fitis_NML, 2,  median), apply(Fitis_NML, 2, mad), apply(Fitis_PL, 2, median), apply(Fitis_PL, 2, mad),
                apply(sqrt_diagGis_PL, 2, median),  apply(Fitis_WPL, 2, median), apply(Fitis_WPL, 2, mad),
